  ...
  <NuxtLayout name="auth">
    <template #image>
      <img src="https://images.unsplash.com/photo-1573164713714-d95e436ab8d6?q=80&w=2069&auto=format&fit=crop"
        alt="Technical Classroom" class="w-full h-full object-cover opacity-30" />
    </template>
    <template #quote>
      <blockquote class="text-2xl font-medium text-white mb-6">
        "Rejoignez notre équipe de formateurs d'élite pour façonner l'avenir de la tech en Afrique."
      </blockquote>
    </template>
    <template #footer>
      <div class="flex items-center gap-4">
        <div class="h-1 w-12 bg-[var(--color-primary)] rounded-full"></div>
        <p class="text-slate-400 font-medium tracking-wide text-sm uppercase">L'équipe Pédagogique</p>
      </div>
    </template>
    ...
    <div class="w-full max-w-md mx-auto">
      <div class="mb-8">
        ...
        <p class="text-slate-500 dark:text-slate-400">
          Finalisez la configuration de votre compte formateur en définissant votre mot de passe.
        </p>
      </div>
      ...
      <form @submit.prevent="handleSubmit" class="space-y-6">
        <!-- Champs pré-remplis (visuels uniquement) -->
        ...
        <div>
          ...
          <p class="text-xs text-gray-500 mt-1">10 car. min, 2 maj, 2 min, 2 chiffres, 2 car. spéciaux</p>
        </div>
...
// Pas de layout pour le formulaire d'invitation
definePageMeta({
  layout: false,
});
...
// Activation du compte formateur
const handleSubmit = async () => {
  ...
  try {
    const response: any = await (api as any)('/formateur/auth/accept-invitation', {
      method: 'POST',
      body: {
        token: token.value,
    ...
    if (response.token) {
      ...
      useToast().success('Compte activé avec succès !');
    ...
    // Afficher l'erreur directement dans l'UI au lieu d'un toast
...
// Récupération des infos de l'invitation à l'ouverture
onMounted(async () => {
  if (!token.value) {
    // ici on garde le toast car on va rediriger
  ...
  try {
    const info: any = await (api as any)(`/formateur/auth/invitation-info?token=${token.value}`);
